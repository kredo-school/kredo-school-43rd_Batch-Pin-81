...
class BookingController extends Controller
{
    ...
    public function create(Request $request, Restaurant $restaurant): View
    {
        ...
        $partySize = max((int) $request->input('guests', 1), 1);
        $selectedTime = $request->string('time')->toString() ?: null;
        $availableSlots = $this->availabilityService->generateAvailableStartTimes(
            $restaurant,
            $selectedDate,
            $partySize
        );

        if ($selectedTime && !in_array($selectedTime, $availableSlots, true)) {
            $selectedTime = null;
        }

        $maxPartySize = (int) ($restaurant->tables()->where('is_active', true)->max('capacity') ?? 0);
    ...
    public function availability(Request $request, Restaurant $restaurant): JsonResponse
    {
        ...
        return response()->json([
            'slots' => $this->availabilityService->generateAvailableStartTimes(
                $restaurant,
                $validated['date'],
                (int) $validated['guests']
            ),
            'duration_minutes' => $this->availabilityService->stayMinutes($restaurant),
            'cleaning_minutes' => ReservationAvailabilityService::CLEANING_MINUTES,
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($start->minute % ReservationAvailabilityService::SLOT_INTERVAL_MINUTES !== 0) {
            throw ValidationException::withMessages([
                'reservation_time' => 'Reservations are available in 15-minute intervals.',
            ]);
        }

        if (!$this->availabilityService->isWithinOperatingHours(
            $restaurant,
        ...
        $reservation = DB::transaction(function () use ($validated, $restaurant) {
            ...
            $status = $this->availabilityService->statusForStart(
                $validated['reservation_date'],
                $validated['reservation_time']
            );
            ...
            return Reservation::create([
                ...
                'end_time' => Carbon::parse($validated['reservation_date'] . ' ' . $validated['reservation_time'])
                    ->addMinutes($this->availabilityService->stayMinutes($restaurant))
                    ->format('H:i:s'),
        ...
        if ($restaurantOwner) {
            $restaurantOwner->notify(new ReservationSubmittedNotification($reservation));
        }
    ...
    {
        $reservation->load(['restaurant', 'user', 'table']);

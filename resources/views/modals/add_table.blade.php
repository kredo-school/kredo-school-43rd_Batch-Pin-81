<div class="modal fade" id="addTableModal" tabindex="-1" aria-labelledby="addTableModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 450px;">
        <div class="modal-content border-0 rounded-4 shadow">
            ...
            <form action="#" method="POST" id="addTableForm">
                @csrf
                <div class="modal-body px-4 py-2">
                    ...
                    <div class="mb-2">
                        ...
                        <input type="number" class="form-control rounded-3" id="addTableCapacityInput" name="capacity" min="1" max="20" placeholder="e.g. 4" required style="border-color: #cbd5e1; padding: 10px 12px;">
                    </div>
                </div>

                <div class="modal-footer border-0 px-4 pb-4 pt-3 d-flex gap-2">
                    <button type="button" class="btn btn-cancel-custom rounded-3 flex-grow-1 fw-bold" style="height: 44px; font-size: 14px;" data-bs-dismiss="modal">
                        Cancel
                    </button>
                    
                    <button type="submit" class="btn btn-add-custom rounded-3 flex-grow-1 fw-bold" style="height: 44px; font-size: 14px;">
                        Add Table
                    </button>
                </div>
            </form>
...
<style>
    ...
    /* ホバー時に少し深みを持たせる（お好みで調整してください） */
    .btn.btn-add-custom:hover {
        background-color: #fbcfe8 !important; /* 少し濃いピンクオブジェクト */
        color: #0A2540 !important;
    }

    /* 💡 通常時：ネイビー（#0A2540）の枠線・文字のクリーンなCancelボタンオブジェクト */
    .btn.btn-cancel-custom {
        color: #0A2540 !important;
        background-color: #ffffff !important;
        border: 1px solid #0A2540 !important;
        transition: all 0.2s ease-in-out;
    }
    /* ホバー時にネイビーベタ・白文字に反転するオブジェクト定義 */
    .btn.btn-cancel-custom:hover {

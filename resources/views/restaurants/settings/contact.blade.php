        ...
        <div class="tab-pane fade" id="message-history" role="tabpanel">
                                    ...
                                    <div>
                                        ...
                                        <div class="d-flex align-items-center gap-2 small text-muted">
                                            ...
                                            @if ($contact->replies && $contact->replies->count() > 0)
                                                <span class="badge bg-success-subtle text-success px-2 py-1 rounded-pill">replied</span>
                                            @else
                                                <span class="badge bg-secondary text-white px-2 py-1 rounded-pill">open</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
            ...
            <div id="history-detail-view" class="support-card p-4 d-none">
                <div class="d-flex align-items-center mb-4 justify-content-between">
                    <div class="d-flex align-items-center">
                        <button type="button" id="btn-back-to-list" class="btn btn-back p-0 fw-normal me-2">← Back</button>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.history-item').forEach(item => {
            item.addEventListener('click', function(e) {
                if (e.target.closest('.delete-history-form') || e.target.closest('.btn-delete-history')) {
                    return;
                }
                e.preventDefault();
                ...
                replies.forEach(reply => {
                    ...
                    if (reply.created_at && reply.created_at.includes('T')) {
                        try {
                            replyTime = new Date(reply.created_at).toLocaleString('ja-JP', {
                                timeStyle: 'short',
                                dateStyle: 'medium'
                            }).replace(/\//g, '-');
                        } catch (err) {
        ...
        function appendChatBubble(sender, text, time, images) {
            ...
            if (imageArray.length > 0) {
                ...
                imageArray.forEach(path => {
                    if (path && typeof path === 'string') {
                        ...
                        imgHtml += `<img src="/storage/${cleanPath}" class="chat-img-preview border" style="max-width: 120px; max-height: 120px; object-fit: cover; cursor: pointer; border-radius: 8px;" onclick="viewImage('/storage/${cleanPath}')">`;
                    }
                });
                imgHtml += '</div>';
            }
            ...
            if (sender === 'user') {
                ...
                bubble.className = 'align-self-start text-start w-100 d-flex flex-column align-items-start mb-2';
                bubble.style.maxWidth = '80%';
                bubble.innerHTML = `
